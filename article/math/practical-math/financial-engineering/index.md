...
$$
sum C_i lr((1+(R)/(m)))^(-i) + F lr((1+(R)/(m)))^(-n)
$$
...
$$
"NPV" = sum_(i=1)^N ("CF"(i))/((1+R)^i) + "CF"(0) = sum_(i=0)^N ("CF"(i))/((1+R)^i)
$$
...
$$
R_(t+1) = (P_(t+1)-P_t+D_(t+1))/(P_t)
$$

対数収益率は

$$
R^("log")_(t+1)=log (P_(t+1)+D_(t+1)) - log P_t tilde.eq R_(t+1) (R_(t+1) << 1)
$$
