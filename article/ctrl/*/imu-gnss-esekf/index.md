...
$$
newcommand{upright("rot")}{upright(upright("Rot"))}
newcommand{w}{omega}
$$

## カルマンフィルタの基礎

- 状態方程式: $x = Fx + Gw$, $w tilde N(0,Q)$
  - $x$: 状態
  - $P$: 状態の共分散行列
  - $F$: 状態遷移モデル
  - $w$: ホワイトノイズ
...
$$
x_(t|t) -> x_(t+1|t) -> x_(t+2|t)
$$

$$
P_(t|t) prec.eq P_(t+1|t) prec.eq P_(t+2|t)
$$
...
$$
x_t = x plus.circle delta x
$$

ここで演算子 $plus.circle$ は、状態の「自然な足し算」を表す。具体的には、位置に対しては並進（ベクトルの加算）を、姿勢に対しては回転（回転群の積）を意味する。

状態方程式も、ノミナル状態方程式と誤差状態方程式に分解する。

$$
f(x_t) =  f(x plus.circle delta x)= f(x) plus.circle f_e(x,delta x)
$$
...
誤差状態方程式は、$O(delta x^2) -> 0$ とみなして線形化できる。

### 観測方程式

$$
z = h(x_t) = h(x plus.circle delta x)
$$

誤差状態カルマンフィルタでは、観測をもとに誤差状態を補正する。

$$
H := lr( (partial h)/(partial delta x)|)_x
$$

これは連鎖律から求めることができる。

$$
lr( (partial h)/(partial delta x)|)_x = lr( (partial h)/(partial x_t)|)_x lr( (partial x_t)/(partial delta x)|)_x =: H_x X_(delta x)
$$
...
| 姿勢                 | $q$   | $dot(q) = (1)/(2)q w$          | $delta theta$ | $dot(delta theta) = upright("rot")(delta theta)   upright("rot")(q)   (w + delta w)$ |
| 速度                 | $v$   | $dot(v) = upright("rot")(q) a$ | $delta v$     | $dot(delta v) = upright("rot")(delta theta)   upright("rot")(q)   (a + delta a)$     |
| 加速度               | $a$   |                                | $delta a$     |                                                                                      |
